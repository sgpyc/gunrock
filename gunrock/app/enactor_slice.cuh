...
struct EnactorSlice
{
        ...
        int num_split_streams = 0)
    {
        ...
        if (num_subq__streams != 0)
        {
            ...
            if (retval = subq__core_dones  .Allocate(num_subq__streams)) return retval;
        ...
        double temp_factor   = 0.1)
    {
        ...
        if (num_input_streams != 0)
        {
            ...
            for (int i=0; i<2; i++)
            {
                ...
                input_queues[i].input_iteration = (i==0)?2:1;
                input_queues[i].output_iteration = (i==0)?2:1;
                input_queues[i].iteration_jump = 2;
