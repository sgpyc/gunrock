    ...
    Problem *problem)
{
    ...
    for (int gpu = 0; gpu < num_gpus; gpu++)
    {
        if (num_gpus > 1)
        {
            ...
            {
                str = str + " #keys" +
                    std::to_string(gpu) + ",0\t #keys"+
                    std::to_string(gpu) + ",1\t #ins" + 
                    std::to_string(gpu) + ",0\t #ins" + 
                    std::to_string(gpu) + ",1";
            }
            else
            {
    ...
    if (num_gpus > 1)
    {
        str = str + " #keys" + std::to_string(num_gpus);
    }
    util::PrintMsg(str);
    ...
    for (int gpu = 0; gpu < num_gpus; gpu++)
    {
        ...
        str = str + "GPU_" + std::to_string(gpu_idx[gpu]) +
            "\t " + std::to_string(org_size[gpu] - gpu_free);
        for (int i = 0; i < num_gpus; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                double factor = 1.0 * x / (num_gpus > 1 ? 
        ...
        if (num_gpus > 1)
        {
            str = str + "\t " + std::to_string(problem -> data_slices[gpu] ->
                frontier_queues[num_gpus].keys[0].GetSize());
        }
        util::PrintMsg(str);
    }
    str = "";
    str = str + "\t queue_sizing =\t " + std::to_string(max_queue_sizing_[0]) + " \t " +
        std::to_string(max_queue_sizing_[1]);
    if (num_gpus > 1)
    {
        str = str + "\t in_sizing =\t " + std::to_string(max_in_sizing_);
    }
    util::PrintMsg(str);
}
