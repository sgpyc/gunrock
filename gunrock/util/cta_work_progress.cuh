...
public:
    ...
    __device__ __forceinline__ void Reset()
    {
        ...
        if (CTA_DEBUG) printf("Reset: d_counters[%d] -> 0\n", threadIdx.x);
    }
    ...
    __device__ __forceinline__ SizeT Steal(int count)
    {
        ...
        if (CTA_DEBUG) printf("Steal1: d_counters[%d] += %d\n", QUEUE_COUNTERS + progress_selector, count);
    ...
    {
        SizeT* d_steal_counters = ((SizeT*) d_counters) + QUEUE_COUNTERS;
        if (CTA_DEBUG) printf("Steal2: d_counters[%d] += %d\n", QUEUE_COUNTERS + (iteration & 1), count);
    ...
    __device__ __forceinline__ void PrepResetSteal()
    {
        SizeT   reset_val = 0;
        SizeT*  d_steal_counters = ((SizeT*) d_counters) + QUEUE_COUNTERS;
        if (CTA_DEBUG) printf("PrepResetSteal1: d_counters[%d] -> 0\n", QUEUE_COUNTERS + (progress_selector ^1));
        util::io::ModifiedStore<util::io::st::cg>::St(
    ...
    __device__ __forceinline__ void PrepResetSteal(IterationT iteration)
    {
        SizeT   reset_val = 0;
        SizeT*  d_steal_counters = ((SizeT*) d_counters) + QUEUE_COUNTERS;
        if (CTA_DEBUG) printf("PrepResetSteal2: d_counters[%d] -> 0\n", QUEUE_COUNTERS + (iteration & 1));
        util::io::ModifiedStore<util::io::st::cg>::St(
    ...
    {
        if (CTA_DEBUG) printf("GetQueueCounter: d_counters[%d] = %d\n", ((SizeT*)d_counters)[iteration&3]);
    ...
    __device__ __forceinline__ SizeT LoadQueueLength(IterationT iteration)
    {
        ...
        if (CTA_DEBUG) printf("LoadQueueLength: iteration = %d, queue_length = %d\n", iteration, queue_length);
        return queue_length;
    }
    ...
    __device__ __forceinline__ void StoreQueueLength(SizeT queue_length, IterationT iteration)
    {
        util::io::ModifiedStore<util::io::st::cg>::St(
            queue_length, GetQueueCounter<SizeT>(iteration));
        if (CTA_DEBUG) printf("StoreQueueLength: iteration = %d, queue_length = %d\n", iteration, queue_length);
    }
    ...
    {
        if (CTA_DEBUG) printf("Enqueue: iteration = %d, count = %d\n", iteration, count);
    ...
    {
        if (CTA_DEBUG) printf("SetOverflow: d_counters[%d] -> 1\n", QUEUE_COUNTERS + STEAL_COUNTERS);
...
public:
    ...
    cudaError_t Init()
    {
        ...
        do {
            ...
            if (!d_counters) {
                ...
                if (CTA_DEBUG) printf("Init: d_counters[] -> 0\n");
            }
        ...
        cudaStream_t stream = 0)
    {
        ...
        if (CTA_DEBUG) printf("CPUSetQueueLength: iteration = %d, queue_length = %d\n", iteration, queue_length);
        return retval;
    }
};
